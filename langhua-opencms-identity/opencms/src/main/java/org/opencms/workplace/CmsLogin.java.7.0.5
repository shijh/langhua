...
import org.langhua.opencms.identity.ldap.openldap.CmsOpenLDAPAuthenticationHandler;
...
public class CmsLogin extends CmsJspLoginBean {
    ...
    public String displayDialog() throws IOException {
        ...
        if (cms.getRequestContext().currentUser().isGuestUser()) {
            ...
            // if module org.langhua.opencms.identity exists, run preLogin
            // added by Shi Yusen, shiys@langhua.cn
            ...
            if (identityModule != null) {
                String className = identityModule
                        .getParameter(I_CmsAuthenticationHandler.AUTH_HANDLER, CmsOpenLDAPAuthenticationHandler.class.getName());
                try {
                    Class handlerClass = Class.forName(className);
                    I_CmsAuthenticationHandler authenticationHandler = (I_CmsAuthenticationHandler) handlerClass
                            .newInstance();
                    authenticationHandler.preLogin(this, identityModule);
        ...
        if (Boolean.valueOf(m_actionLogin).booleanValue()) {
            ...
            if ((m_username == null) && (m_password == null)) {
                ...
                if (getLoginException() == null) {
                    ...
                    // if module org.langhua.opencms.identity exists, get the identity config
                    // added by Shi Yusen, shiys@langhua.cn
                    CmsModule identityModule = OpenCms.getModuleManager()
                            .getModule(I_CmsAuthenticationHandler.IDENTITY_MODULE_NAME);
                    if (identityModule != null && m_message == null) {
                        String className = identityModule
                                .getParameter(I_CmsAuthenticationHandler.AUTH_HANDLER, CmsOpenLDAPAuthenticationHandler.class.getName());
                        try {
                            Class handlerClass = Class.forName(className);
                            I_CmsAuthenticationHandler authenticationHandler = (I_CmsAuthenticationHandler) handlerClass
                                    .newInstance();
                            m_message = authenticationHandler.getMessage(cms,
                                    m_username, getLoginException()
            ...
            // if module org.langhua.opencms.identity exists, run preLogout first
            // added by Shi Yusen, shiys@langhua.cn
            CmsModule identityModule = OpenCms.getModuleManager().getModule(
                    I_CmsAuthenticationHandler.IDENTITY_MODULE_NAME);
            if (identityModule != null) {
                ...
                try {
                    ...
                    I_CmsAuthenticationHandler authenticationHandler = (I_CmsAuthenticationHandler) handlerClass
                            .newInstance();
                    authenticationHandler.preLogout(this, identityModule);
                } catch (ClassNotFoundException e) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug(e);
                    }
