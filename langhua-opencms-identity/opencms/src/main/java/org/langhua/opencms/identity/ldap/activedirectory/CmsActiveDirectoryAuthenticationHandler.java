...
package org.langhua.opencms.identity.ldap.activedirectory;
...
/**
 * The OpenCms ActiveDirectory Authentication Handler.<p>
 * 
 * New login error messages are added.
 *  
 * User                          OpenCms                     ActiveDirectory
 *  |      username/password        |                                |
 *  |------------req--------------->|        search cn=username      |
 *  |                               |---------------req------------->|
 *  |                               |<--------------res--------------|
 *  |                               |             user dn            |
 *  |                               |        user dn/password        |
 ...
 * 
 */
public final class CmsActiveDirectoryAuthenticationHandler extends CmsLdapAuthenticationHandler {

    /**
     * Public constructor, initializes some required member variables.<p> 
     */
    public CmsActiveDirectoryAuthenticationHandler() {
	...
	protected void setUpSecurityCredentials(CmsModule identityModule, Hashtable<String, String> env) {
		String searchType = identityModule.getParameter(LDAP_SEARCH_TYPE, "");
		if (CmsStringUtil.isEmpty(searchType)) {
			env.put(Context.SECURITY_AUTHENTICATION, "none");
		} else if (searchType.trim().equals("login")) {
			env.put(Context.SECURITY_AUTHENTICATION, getParamAuthenType(identityModule));
			// specify the username for search
			String userDNForSearch = identityModule.getParameter(LDAP_USERDN_FOR_SEARCH);
			env.put(Context.SECURITY_PRINCIPAL, userDNForSearch);
			// specify the password for search
			String passwordForSearch = identityModule.getParameter(LDAP_PASSWORD_FOR_SEARCH);
