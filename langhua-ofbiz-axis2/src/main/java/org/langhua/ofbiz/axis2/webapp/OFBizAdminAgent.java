 ...
 * Copied from axis2 webapp module.
 */
public class OFBizAdminAgent extends AbstractAgent {
	
    public final static String module = OFBizAdminAgent.class.getName();
    ...
    public OFBizAdminAgent(ConfigurationContext aConfigContext) {
    ...
    public void processLogin(HttpServletRequest req, HttpServletResponse res)
        ...
        String result = LoginWorker.checkLogin(req, res);
        if (result.equals("success")) {
            req.getSession().setAttribute(Constants.LOGGED, "Yes");
            renderView(ADMIN_JSP_NAME, req, res);
        } else {
        	String errorMsg = (String) req.getAttribute("_ERROR_MESSAGE_");
            req.setAttribute("errorMessage", "Invalid auth credentials!" + (UtilValidate.isEmpty(errorMsg) ? "" : (" " + errorMsg)));
            renderView(LOGIN_JSP_NAME, req, res);
        }
    }
